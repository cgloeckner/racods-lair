...
Application<Context>::Application(Args&&... args)
...
template <typename Context>
std::size_t Application<Context>::getDeepest() const {
	auto i = states.size() - 1u;
	for (; i >= 0; --i) {
...
template <typename Context>
Context& Application<Context>::getContext() {
	return context;
}

template <typename Context>
Context const & Application<Context>::getContext() const {
	return context;
}
...
void Application<Context>::run() {
	...
	while (window.isOpen()) {
		...
		// draw transparent states from deepest to top
		auto deepest = getDeepest();
		for (auto i = deepest; i < states.size(); ++i) {
...
template <typename Context>
State<Context>::State(Application<Context>& application)
...
template <typename Context>
void State<Context>::setTransparency(bool flag) {
	transparent = flag;
}

template <typename Context>
bool State<Context>::isTransparent() const {
	return transparent;
}
