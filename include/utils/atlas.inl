...
Chunk<Key>::Chunk(Key const& key, sf::Image&& img, sf::IntRect const& bounds,
...
template <typename Key>
void AtlasGenerator<Key>::add(
	Key const& key, sf::Image&& image, sf::Vector2f origin) {
...
bool AtlasGenerator<Key>::generate(sf::Vector2u const& min_step,
	std::size_t size, Atlas<Key, HashFunc>& atlas) {
	...
	for (auto& chunk : chunks) {
		...
		for (chunk.target.top = 0u; chunk.target.top <= max_top;
			 chunk.target.top += step_range.y) {
			for (chunk.target.left = 0u; chunk.target.left <= max_left;
				 chunk.target.left += step_range.x) {
				...
				if (found) {
					...
					step_range.x =
						boost::math::gcd(step_range.x, chunk.bounds.width);
					step_range.y =
						boost::math::gcd(step_range.y, chunk.bounds.height);

					// note: this might speedup generation, but the result won't
					// be perfect anymore
					step_range.x =
						std::max(step_range.x, static_cast<int>(min_step.x));
					step_range.y =
						std::max(step_range.y, static_cast<int>(min_step.y));
	...
	for (auto const& chunk : chunks) {
		atlas.image.copy(
			chunk.image, chunk.target.left, chunk.target.top, chunk.bounds);
		AtlasFrame frame;
