...
template <GridMode M>
TilingIterator<M>::TilingIterator(
	sf::Vector2i const& start, sf::Vector2i const& range)
	: start{start}, range{range}, current{start}, count{0u} {}
...
inline void TilingIterator<GridMode::IsoDiamond>::operator++() {
	...
	++current.x;
	++count;
	if (static_cast<int>(count) >= range.x) {
...
Tiling<M>::Tiling(sf::Vector2f const& tile_size)
	: view{}, tile_size{tile_size}, padding{0u, 0u} {}
...
inline sf::Vector2u Tiling<GridMode::Orthogonal>::getRange() const {
	...
	// calculate range
	// size + 2 : because tile might be rendered centered (else: gap at
	// bottom/right)
	// note: two casts necessary to avoid undefined behavior
	range.x = static_cast<unsigned int>(
				  static_cast<int>(std::ceil(size.x / tile_size.x))) +
			  2;
	range.y = static_cast<unsigned int>(
				  static_cast<int>(std::ceil(size.y / tile_size.y))) +
			  2;
...
// specialization for orthogonal maps
template <>
inline sf::Vector2f Tiling<GridMode::Orthogonal>::toScreen(
	sf::Vector2f const& world_pos) const {
	return {world_pos.x * tile_size.x, world_pos.y * tile_size.y};
}

// specialization for isometric (diamond) maps
template <>
inline sf::Vector2f Tiling<GridMode::IsoDiamond>::toScreen(
	sf::Vector2f const& world_pos) const {
	return {(world_pos.x - world_pos.y) * tile_size.x / 2.f,
		(world_pos.x + world_pos.y) * tile_size.y / 2.f};
}

// specialization for orthogonal maps
template <>
inline sf::Vector2f Tiling<GridMode::Orthogonal>::fromScreen(
	sf::Vector2f const& screen_pos) const {
	return {screen_pos.x / tile_size.x, screen_pos.y / tile_size.y};
}

// specialization for isometric (diamond) maps
template <>
inline sf::Vector2f Tiling<GridMode::IsoDiamond>::fromScreen(
	sf::Vector2f const& screen_pos) const {
	return {(screen_pos.x / (tile_size.x / 2.f) +
				screen_pos.y / (tile_size.y / 2.f)) /
				2.f,
		(screen_pos.y / (tile_size.y / 2.f) -
			screen_pos.x / (tile_size.x / 2.f)) /
			2.f};
}
...
inline sf::Vector2i Tiling<GridMode::Orthogonal>::getTopleft() const {
	...
	// note: two casts necessary to avoid undefined behavior
	topleft.x = static_cast<unsigned int>(
		static_cast<int>(center.x - std::ceil(range.x / 2.f)));
	topleft.y = static_cast<unsigned int>(
		static_cast<int>(center.y - std::ceil(range.y / 2.f)));
