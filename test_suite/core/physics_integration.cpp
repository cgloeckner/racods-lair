...
struct PhysicsFixture
	...
	PhysicsFixture()
		...
		scene = dungeon.create(dummy_tileset, grid_size, sf::Vector2f{1.f, 1.f});
...
BOOST_AUTO_TEST_CASE(bullet_stops_movement_if_it_hits_an_unaccessable_tile) {
	...
	BOOST_CHECK_LT(b_m.pos.x, 1.5f);
	BOOST_CHECK_CLOSE(b_m.pos.y, 2.f, 0.00001f);
}
...
BOOST_AUTO_TEST_CASE(
	...
	BOOST_CHECK_CLOSE(m_m.pos.x, 3.f, 0.0001f);
	BOOST_CHECK_LT(m_m.pos.y, 3.f);
}
...
BOOST_AUTO_TEST_CASE(direct_tunnelingtest_fast) {
	auto& fix = Singleton<PhysicsFixture>::get();
	fix.reset();
	...
	// expect actor has stopped right of (2, 2)
	auto const& m_m = fix.movement.query(mover);
	BOOST_CHECK_VECTOR_CLOSE(m_m.move, sf::Vector2f(), 0.0001f);
	BOOST_CHECK_GT(m_m.pos.x, 2.f);
	BOOST_CHECK_CLOSE(m_m.pos.y, 2.f, 0.0001f);
	
	// and expect oncomming object has stopped left of (8, 2)
	auto const& o_m = fix.movement.query(oncom);
	BOOST_CHECK_VECTOR_CLOSE(o_m.move, sf::Vector2f(), 0.0001f);
	BOOST_CHECK_LT(o_m.pos.x, 8.f);
	BOOST_CHECK_CLOSE(o_m.pos.y, 2.f, 0.0001f);
	
	// expect mover to be left of oncom
	BOOST_CHECK_LT(m_m.pos.x, o_m.pos.x);
...
BOOST_AUTO_TEST_CASE(direct_tunnelingtest_slow) {
	auto& fix = Singleton<PhysicsFixture>::get();
	fix.reset();

	auto mover = fix.add_object(fix.scene, {1u, 2u}, {-1, 0}, 1.f, 2.f);
	...
	// expect actor has stopped right of (4, 2)
	auto const& m_m = fix.movement.query(mover);
	BOOST_CHECK_VECTOR_CLOSE(m_m.move, sf::Vector2f(), 0.0001f);
	BOOST_CHECK_GT(m_m.pos.x, 4.f);
	BOOST_CHECK_CLOSE(m_m.pos.y, 2.f, 0.0001f);
	
	// and expect oncomming object has stopped left of (6, 2)
	auto const& o_m = fix.movement.query(oncom);
	BOOST_CHECK_VECTOR_CLOSE(o_m.move, sf::Vector2f(), 0.0001f);
	BOOST_CHECK_LT(o_m.pos.x, 6.f);
	BOOST_CHECK_CLOSE(o_m.pos.y, 2.f, 0.0001f);
...
BOOST_AUTO_TEST_CASE(indirect_tunnelingtest_fast) {
	auto& fix = Singleton<PhysicsFixture>::get();
	fix.reset();
	...
	// and expect oncomming object has stopped left of (8, 3)
	auto const& o_m = fix.movement.query(oncom);
	BOOST_CHECK_VECTOR_CLOSE(o_m.move, sf::Vector2f(), 0.0001f);
	BOOST_CHECK_LT(o_m.pos.x, 8.f);
	BOOST_CHECK_CLOSE(o_m.pos.y, 3.f, 0.0001f);
	
	// expect mover to be left of oncom
	BOOST_CHECK_LT(m_m.pos.x, o_m.pos.x);
	
	/// @note because of the high speed, testing the distances does not
	///		indicate tunneling/collision. hence it's skipped here
	///		see `indirect_tunnelingtest_slow` for an example
}

BOOST_AUTO_TEST_CASE(indirect_tunnelingtest_slow) {
	auto& fix = Singleton<PhysicsFixture>::get();
	fix.reset();

	auto mover = fix.add_object(fix.scene, {1u, 2u}, {-1, 0}, 1.f, 2.f);
	auto oncom = fix.add_object(fix.scene, {9u, 3u}, {-1, 0}, 1.f, 2.f);
	auto& m_c = fix.collision.query(mover);
	auto& o_c = fix.collision.query(oncom);
	m_c.shape.radius = 0.75f;
	o_c.shape.radius = 0.75f;
	...
	// and expect oncomming object has stopped left of (6, 3)
	auto const& o_m = fix.movement.query(oncom);
	BOOST_CHECK_VECTOR_CLOSE(o_m.move, sf::Vector2f(), 0.0001f);
	BOOST_CHECK_LT(o_m.pos.x, 6.f);
	BOOST_CHECK_CLOSE(o_m.pos.y, 3.f, 0.0001f);
	...
	// check distance (that they are close enough to actually collide)
	auto dist = utils::distance(m_m.pos, o_m.pos); // distance between both is squared
	auto radsum = m_c.shape.radius + o_c.shape.radius; // distance for colliding
	BOOST_CHECK_LT(dist, radsum * radsum);
}
...
BOOST_AUTO_TEST_CASE(
	...
	// expect actor has right of (3, 2)
	auto const& m_m = fix.movement.query(mover);
	BOOST_CHECK_VECTOR_EQUAL(m_m.move, sf::Vector2i());
	BOOST_CHECK_GT(m_m.pos.x, 3.f);
	BOOST_CHECK_CLOSE(m_m.pos.y, 2.f, 0.00001f);
	
	// and expect oncomming object has stopped left of (4.5, 2)
	auto const& o_m = fix.movement.query(oncom);
	BOOST_CHECK_VECTOR_EQUAL(o_m.move, sf::Vector2i());
	BOOST_CHECK_LT(o_m.pos.x, 4.5f);
	BOOST_CHECK_CLOSE(o_m.pos.y, 2.f, 0.00001f);
	
	// expect mover to be left from oncom
	BOOST_CHECK_LT(m_m.pos.x, o_m.pos.x);
...
BOOST_AUTO_TEST_CASE(collision_map_works_correct_if_object_collides_with_dungeon_border) {
	...
	// expect some suitable position (0 < x < 1.5)
	BOOST_CHECK_GT(move.pos.x, 0.f);
	BOOST_CHECK_LT(move.pos.x, 1.5f);
...
BOOST_AUTO_TEST_CASE(object_is_not_stopped_after_teleport) {
	...
	BOOST_CHECK_CLOSE(move_data.pos.y, 5.f, 0.0001f);
}
...
BOOST_AUTO_TEST_CASE(bullet_is_not_effected_by_teleport) {
	...
	BOOST_CHECK_GT(move_data.pos.x, 8.f);
	BOOST_CHECK_CLOSE(move_data.pos.y, 1.f, 0.0001f);
}
